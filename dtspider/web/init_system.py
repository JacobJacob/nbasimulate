...
def init():
    ...
    sys.modules["dtspider"] = project_module
    del sys.path[0]

    #set django path
    for k in [k for k in sys.modules if k.startswith('django')]:
        del sys.modules[k]
    import dtspider.common.django as _local_django
    sys.modules["django"] = _local_django
    
    #set flup path
    for k in [k for k in sys.modules if k.startswith('flup')]:
        del sys.modules[k]
    from dtspider.common import flup as _local_flup
